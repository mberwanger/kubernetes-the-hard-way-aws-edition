...
{
  "CN": "system:node:${PRIVATE_DNS_NAME}",
  "key": {
    "algo": "rsa",
    "size": 2048
  },
...
PRIVATE_ADDRESS=$(aws ec2 describe-instances \
  --output text \
  --filters "{\"Name\":\"tag:Name\", \"Values\":[\"${instance}\"]}" "{\"Name\":\"instance-state-name\", \"Values\":[\"running\"]}" \
...
The `kubernetes-the-hard-way` elastic load balancer will be included in the list of subject alternative names for the Kubernetes API Server certificate. This will ensure the certificate can be validated by remote clients.
...
cfssl gencert \
  ...
  -ca-key=ca-key.pem \
  -config=ca-config.json \
  -hostname=10.32.0.1,10.240.0.10,10.240.0.11,10.240.0.12,${KUBERNETES_ENDPOINT},127.0.0.1,kubernetes.default \
  -profile=kubernetes \
...
```
for instance in worker-0 worker-1 worker-2; do
  scp -i ~/.ssh/kubernetes-the-hard-way.pem ca.pem ${instance}-key.pem ${instance}.pem ubuntu@$(ip_from_instance "$instance"):~/
done
```
